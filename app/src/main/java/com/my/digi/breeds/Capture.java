...
public class Capture extends Activity implements View.OnClickListener, TextToSpeech.OnInitListener {
	...
	String[] objectsArray, objectsMidArray;
	...
	TextView tvObjectDetails, tvProfile, tvDogBreedName, tvInstructions, tvProfileName, tvProfileBackground;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tvDogBreedName.setVisibility(View.INVISIBLE);
	...
	private void initialize() {
		...
		tvDogBreedName = (TextView) findViewById(R.id.tvDogBreedName);
		//tvDogBreedName.bringToFront();
		tvProfile = (TextView) findViewById(R.id.tvProfile);
		tvProfileName = (TextView) findViewById(R.id.tvProfileName);
		tvProfileBackground = (TextView) findViewById(R.id.tvProfileBackground);
		tvInstructions = (TextView) findViewById(R.id.tvInstructions);
		tvInstructions.setText(" Want to know the breed of a dog? \n " +
				"		a. Point camera phone at dog \n" +
				"		b. Draw box around dog (OPTIONAL) \n" +
				"		c. Click Search icon \n" +
				"										OR \n" +
				"		a. Upload an image of a dog \n");

		ivPreviewProfileCircle.setOnClickListener(this);
		tvDogBreedName.setOnClickListener(this);
		...
		tvDogBreedName.setTypeface(font);
	}
	...
	public void onClick(View v){
		switch (v.getId()){
			...
			//Click to show dog profile
			case R.id.ivPreviewProfileCircle:
				if (breedFound){
					ShowProfile();
				}
				break;
			...
			case R.id.bClose:
				...
				//tvDogBreedName.setVisibility(View.VISIBLE);
				//tvProfile.setVisibility(View.INVISIBLE);
				//bPlay.setVisibility(View.VISIBLE);
				//bClose.setVisibility(View.INVISIBLE);
				//ivPreview.setVisibility(View.VISIBLE);
				//ivPreviewProfile.setVisibility(View.INVISIBLE);
				//ivPreviewProfileCircle.setVisibility(View.VISIBLE);
				Clear();
			...
			case R.id.bPlaySound:
				if (breedFound){
					...
					tts.speak(database.getBreed(breedIndex), TextToSpeech.QUEUE_FLUSH, null);
				}
				break;

			//Goes to browse activity
			case R.id.bBrowse:
				Intent intent = new Intent("com.my.digi.breeds.BROWSE");
			...
			protected void onPostExecute(String result) {
				...
				if (objectsArray != null){
					...
					for (int i=0; i < objectsArray.length; i++){
						if (!database.foundBreed(objectsMidArray[i])){
					...
					/*if (Arrays.asList(objectsArray).contains("dog crossbreeds")){
						tvDogBreedName.setVisibility(View.VISIBLE);
						tvDogBreedName.setText("Dog found. Possible crossbreed");
						breedFound = true;
					}*/

					int i = 0;
					while(!breedFound && i < objectsArray.length){
						//Arrays.asList(dogMidArray).contains(objectsMidArray[i])
						if (database.foundBreed(objectsMidArray[i])) {
							tvDogBreedName.setVisibility(View.VISIBLE);
							ivPreviewProfileCircle.setVisibility(View.VISIBLE);

							//breedIndex = Arrays.asList(dogMidArray).indexOf(objectsMidArray[i]);
							breedIndex = database.getIndex(objectsMidArray[i]);
							//tvDogBreedName.setText(String.format("%.2f", objectsScoreArray[i]*100) + "%\n " + dogBreedArray[breedIndex]);
							//tvDogBreedName.setText(dogBreedArray[breedIndex]);

							//String PACKAGE_NAME = getApplicationContext().getPackageName();
							//int id = getResources().getIdentifier(PACKAGE_NAME+":drawable/" + dogImageNameArray[breedIndex], null, null);
							//ivPreviewProfileCircle.setImageResource(id);
					...
					if (!breedFound){
						if (Arrays.asList(objectsArray).contains("dog")){
							tvDogBreedName.setVisibility(View.VISIBLE);
							tvDogBreedName.setText("Dog found. Breed unknown");
						}
						else{
							tvDogBreedName.setVisibility(View.VISIBLE);
							tvDogBreedName.setText("No dog found in image");
						}
						try {
							writeToFile(Environment.getExternalStorageDirectory().getPath(), "logfileBreeds.txt", objectResults);
						} catch (IOException e) {
							e.printStackTrace();
						}
					}
					...
					tvDogBreedName.setVisibility(View.VISIBLE);
					tvDogBreedName.setText("Please check internet connection");
				}
			}
		}.execute();
		mCamera.startPreview();
	...
	public void ShowProfile(){
		...
		/*tvProfile.setText("Breed Name:           " + dogBreedArray[breedIndex] + "\n" +
				"Breed Category:     " + dogCategoryArray[breedIndex] + "\n" +
				"Lifespan:                 " + dogLifespanArray[breedIndex] + "\n" +
				"Friendliness:           " + dogFriendlinessArray[breedIndex] + "/5 \n" +
				"Playfulness:            " + dogPlayfulnessArray[breedIndex] + "/5");*/
